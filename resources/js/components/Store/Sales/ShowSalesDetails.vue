<template>
	<div>
		<div class="d-flex justify-content-between my-4">
			...
			<div>
				{{ __('app.common.client') }}:
				<span class="font-weight-bold">{{ invoice.client }}</span>
			</div>
			<div>
				{{ __('app.common.discount') }}:
...
export default {
		...
		return {
			detailsFields: [
               ...
               formatter: (value, key, item) => this.price(value)
				...
				{
					key: 'total_price',
					label: this.__('app.common.sum'),
					formatter: (value, key, item) => this.toFixed(item.price * item.quantity),
				},
	...
	methods: {
		destroy(item, index) {
			this.deleteData(`/api/sales/details/${item.id}`).then(res => {
				if (res === 'deleted') {
					eventBus.$emit('delete-sale-detail', index)
				}
			})
		}
	}
